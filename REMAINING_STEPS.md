...
- [ ] Add a simulated camera sensor payload to the Gazebo URDF (`camera_sensor`).
- [ ] Write `vision_node.py` using OpenCV to subscribe to `/camera/image_raw` and detect specific colored blocks.
- [ ] Implement camera calibration module to convert 2D pixel coordinates into 3D world (MoveIt) coordinates.

## 🔵 Phase 6: Autonomous Sorting Logic (AI)
- [ ] Integrate YOLOv8 with PyTorch/CUDA into `vision_node.py` for advanced object classification.
- [ ] Write `sorting_planner_node.py` using MoveIt 2 Python MoveGroup API to mathematically calculate arm endpoints.
...
- [ ] Write `arm_firmware.ino` using the Adafruit PWM Servo library for the PCA9685 I2C driver board.
- [ ] Create ROS 2 package: `robot_arm_hardware`.
